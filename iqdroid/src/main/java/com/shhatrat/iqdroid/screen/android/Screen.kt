...
class Screen private constructor(
    ...
    data class Builder(
        ...
        fun navigation(navigation: MutableMap<Int, Navigation>) = apply {this.navigation = navigation}
        fun screenItemList(screenItemList: MutableList<IqScreenItem>) = apply {this.screenItemList = screenItemList}
        fun build() = Screen(NO_ID, description, localData, navigation?: mutableMapOf(), screenItemList?: mutableListOf())
    }

    fun addId(idToAdd: Int): Screen{
        return Screen(idToAdd, this.description, this.localData, this.navigation, this.screenItemList)
    }
}